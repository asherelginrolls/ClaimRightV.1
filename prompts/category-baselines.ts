...
export const CATEGORY_BASELINES: Record<CanonicalCategory, CategoryBaseline> = {
  ...
  documentation_incomplete: {
    ...
    baselineParagraph: `I respectfully submit that the rejection on the ground of incomplete documentation does not withstand scrutiny. The IRDAI Master Circular on Protection of Policyholders' Interests dated 05.09.2024 categorically provides that no claim shall be rejected or closed for want of documents or for delayed intimation of claim, and the IRDAI Master Circular on Health Insurance dated 29.05.2024 requires that insurers and their Third Party Administrators collect the required documents directly from the hospital, expressly providing that the policyholder shall not be required to submit such documents. ${citeMC('no claim rejected for want of documents')} Where settlement is delayed beyond the mandated timelines, the policyholder is further entitled to penal interest at the bank rate plus two per cent, payable suo-moto by the insurer. ${citeMC('settlement timelines and delay interest')} I therefore request that the claim be reopened and processed in accordance with these binding provisions.`,
    fallbackCitation: {
      regulation_title: MASTER_CIRCULAR_TITLE,
      section: MASTER_CIRCULAR_DATE,
